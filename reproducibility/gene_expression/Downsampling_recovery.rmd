...
```
### Load Data
Here, we use melanoma dataset as an example. The fraction of sampling is set to 50% of the original library size across cells.
```{r}
raw_data = readh5_loom("./data/melanoma/raw.loom")
observed_data = downsampling_cell(0.5, raw_data)
save_h5("./data/melanoma/ds_0.5.loom", t(observed_data))
```
...
for(ii in 1:repeats){
  output_dir = paste0(paste(delete_last_element(unlist(strsplit(loom_path, "/"))), collapse = "/"), "/downsampling_first_repeat_", ii)
  dir.create(output_dir, showWarnings = F)
  for(jj in ds_rate){
    output_loom = paste0(output_dir, "/", sub(".loom", paste0("_ds_", jj, ".loom"), get_last_element(unlist(strsplit(loom_path, "/"))), fixed = TRUE))
    save_h5(output_loom, t(downsampling(jj, gene_bc_mat)))
    cat(ii, " - ", jj, "\n")
  }
}

dataset_list = list()
dataset_list[["FISH"]] = readh5_loom("./imputation_results/melanoma/fish.loom")
raw_input_data = "./imputation_results/melanoma/raw.loom"
...
```{r}
### DISC
our_result = "./imputation_results/melanoma/DISC.hdf5"
dataset_list[["DISC"]] = readh5_imputation(our_result, use_genes, with_outliers=T)
### Other methods
dataset_list[["SAVER"]] = readRDS("./imputation_results/melanoma/SAVER.rds")
set.seed(42)
dataset_list[["SAVER_gamma"]] = gamma_result(dataset_list[["SAVER"]], num_of_obs=1)[use_genes, used_cells]
dataset_list[["MAGIC"]] = readh5_imputation("./imputation_results/melanoma/MAGIC.hdf5", use_genes, used_cells)
dataset_list[["DCA"]] = readh5_imputation("./imputation_results/melanoma/DCA.hdf5", use_genes, used_cells)
dataset_list[["scScope"]] = readh5_imputation("./imputation_results/melanoma/scScope.hdf5", use_genes, used_cells)
dataset_list[["scVI"]] = readh5_imputation("./imputation_results/melanoma/scVI.hdf5", use_genes, used_cells)
```
### Output settings
...
```
### Notes:
Single plots of FF are saved in <a href="https://github.com/iyhaoo/DISC/tree/master/reproducibility/imputation_results/melanoma/structure_recovery/2d_distribution">here</a>.</br>
The imputation results are too large to upload into GitHub and can be reproduced using <a href="https://github.com/iyhaoo/DISC/blob/master/reproducibility/tutorials/run_imputation.md">this script</a>.
