---
title: "Structure recovery comparing with FISH"
---
...
```{r}
generic_functions_path = "./source/generic_functions.r"
source(generic_functions_path)
```
## Load Data
Here, gene expression matrixes of RNA-seq and FISH experiments will be loaded.
We removed cells as SAVER(https://www.nature.com/articles/s41592-018-0033-z).
...
```
After `Run imputation`(https://github.com/iyhaoo/DISC/blob/master/reproducibility/tutorials/imputation.md), we get imputation results. Now we load them.
```{r}
### DISC
...
```
### Notes:
The height and width of result pictures are different from those in PDF files which are saved here(https://github.com/iyhaoo/DISC/tree/master/reproducibility/imputation_results/melanoma/structure_recovery).
The imputation results are too large to upload into GitHub and can be reproduced using this script( https://github.com/iyhaoo/DISC/blob/master/reproducibility/tutorials/imputation.md).
