...
for(ii in used_datasets){
  ...
  for(jj in repeats){
    ...
    data_list[[ii]][[jj]] = list(raw_zero_imputation = DISC_rescale[raw_zero],
                                 ...
                                 induced_zero_value = raw_data[compared_genes, ][induced_zero],
                                 ...
                                 top_induced_zero_value = raw_data[used_top_genes, ][induced_zero[used_top_genes, ]],
...
for(ii in used_datasets){
  for(jj in repeats){
    ...
    density_induced_zero_values = density(data_list[[ii]][[jj]][["induced_zero_value"]][data_list[[ii]][[jj]][["induced_zero_value"]] <= quantile(data_list[[ii]][[jj]][["induced_zero_value"]], 0.99)])
    ...
    y_max = quantile(c(density_raw_zero$y, density_induced_zero$y, density_induced_zero_values$y, density_kept_values$y), 0.99) * 1.02
    x_max = quantile(c(data_list[[ii]][[jj]][["raw_zero_imputation"]], data_list[[ii]][[jj]][["induced_zero_imputation"]], data_list[[ii]][[jj]][["induced_zero_value"]], data_list[[ii]][[jj]][["kept_values_imputation"]]), 0.99) * 1.02
    ...
    lines(density_induced_zero_values, col = "green", lwd = 2)
    axis(1)
  ...
  legend("topright", c("Raw zero", "Sampled zero", "Sampled zero's raw value", "Kept values"), lty = 1, col = c("black", "red", "green", "blue"), lwd = 2)
  cat("Finish: ", ii, "\n")
}
```
### Top 1000 genes
...
for(ii in used_datasets){
  for(jj in repeats){
    ...
    density_induced_zero_values = density(data_list[[ii]][[jj]][["top_induced_zero_value"]][data_list[[ii]][[jj]][["top_induced_zero_value"]] <= quantile(data_list[[ii]][[jj]][["top_induced_zero_value"]], 0.99)])
