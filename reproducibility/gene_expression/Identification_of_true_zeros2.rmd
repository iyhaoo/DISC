...
for(ii in used_datasets){
  ...
  for(jj in method_names){
    if(jj == "DISC"){
      data_list[[ii]][[jj]] = readh5_loom(paste0(ds_dir, "/", jj, ".loom"))[compared_genes, ]
    }else{
      data_list[[ii]][[jj]] = readh5_imputation(paste0(ds_dir, "/", jj, ".hdf5"))[compared_genes, ]
    }
  }
}
```
...
for(ii in method_names){
  for(jj in used_datasets){
    density_true_zero = density(data_list[[jj]][[ii]][data_list[[jj]][["true_zero_mask"]]] / 0.5)
    density_induced_zero = density(data_list[[jj]][[ii]][data_list[[jj]][["induced_zero_mask"]]] / 0.5)
    y_max = max(c(density_true_zero$y, density_induced_zero$y))
    x_max = quantile(data_list[[jj]][[ii]][data_list[[jj]][["Observed_0"]]], 0.999)
    plot(0, type = "n", xlim = c(-min(c(1, x_max * 0.05 * 999999999)), 8), ylim = c(0, y_max), axes = FALSE, ann = FALSE, frame.plot = TRUE)
...
for(ii in method_names){
  for(jj in used_datasets){
    density_true_zero = density(data_list[[jj]][[ii]][data_list[[jj]][["top_1000_genes"]], ][data_list[[jj]][["true_zero_mask"]][data_list[[jj]][["top_1000_genes"]], ]] / 0.5)
    density_induced_zero = density(data_list[[jj]][[ii]][data_list[[jj]][["top_1000_genes"]], ][data_list[[jj]][["induced_zero_mask"]][data_list[[jj]][["top_1000_genes"]], ]] / 0.5)
    y_max = max(c(density_true_zero$y, density_induced_zero$y))
    x_max = quantile(data_list[[jj]][[ii]][data_list[[jj]][["top_1000_genes"]], ][data_list[[jj]][["Observed_0"]][data_list[[jj]][["top_1000_genes"]], ]], 0.999)
    plot(0, type = "n", xlim = c(-min(c(1, x_max * 0.05 * 9999999999)), 8), ylim = c(0, y_max), axes = FALSE, ann = FALSE, frame.plot = TRUE)
    lines(density_true_zero, col = "black", lwd = 2)
    lines(density_induced_zero, col = "red", lwd = 2)
    axis(1)
    mtext(jj, outer = TRUE, cex = 1.2, font = 2, line = 0.5, at = mtext_position[jj])
