...
Firstly, we assign cell types following these 2 scripts (https://github.com/Winnie09/imputationBenchmark/blob/master/data/code/process/06_make_hca_MantonBM6.R and https://github.com/Winnie09/imputationBenchmark/blob/master/data/code/process/07_hca_assign_celltype.R).
#### STEP 1
```{r}
...
sf = sizeFactors(sce)
norm_gene_bcmat = log2(sweep(gene_bc_mat,2,sf,"/") + 1)
```
...
dataset_list[["scImpute"]] = readh5_imputation("./data/MELANOMA/scImpute.hdf5", use_genes, used_cells)
dataset_list[["VIPER"]] = readh5_imputation("./data/MELANOMA/VIPER.hdf5", use_genes, used_cells)
...
method_color = c("gray80", "#FF0000", "#BFBF00", "#408000", "#804000", "#00FF00", "#FF8000", "#FF00FF", "#00FFFF")
...
ALL data we used here can be found <a href="https://github.com/iyhaoo/DISC_data_availability/tree/master/MELANOMA">here</a>.
