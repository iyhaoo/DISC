...
if(!file.exists("./results/10X_5CL/sc_DEG_list.rds")){
  ...
  gene_bc_mat = readh5_loom("./data/10X_5CL/raw.loom")
  sc_celltype = sapply(colnames(gene_bc_mat), function(x){
    return(strsplit(x, "_", fixed = T)[[1]][[1]])
  })
  ...
  for(ii in method_name){
    if(ii == "Raw"){
      ...
      gene_bc_mat = readh5_loom(paste0("./data/10X_5CL/", ii, ".loom"))
      gene_bc_filt = gene_bc_mat[sc_gene, ]
    }
    sc_DEG_list[["wilcox"]][[ii]] = list()
    sc_DEG_list[["MAST"]][[ii]] = list()
    for(jj in seq(ncol(celltype_combination))){
      celltype1 = celltype_combination[1, jj]
      celltype2 = celltype_combination[2, jj]
      used_sample = sc_celltype[sc_celltype %in% c(celltype1, celltype2)]
      this_name = paste(c(celltype1, celltype2), collapse = "_")
      seurat_obj = CreateSeuratObject(counts = as.data.frame(gene_bc_filt[, names(used_sample)]), min.cells = 0, min.features = 0)
      seurat_obj = NormalizeData(seurat_obj, normalization.method = "LogNormalize", scale.factor = 10000)
      seurat_obj@active.ident = cell_type
      suppressWarnings({sc_DEG_list[["wilcox"]][[ii]][[this_name]] = FindMarkers(seurat_obj, ident.1 = celltype1, ident.2 = celltype2, min.pct = 0.1, logfc.threshold = 0, test.use = "wilcox", verbose = F)})
      suppressWarnings({sc_DEG_list[["MAST"]][[ii]][[this_name]] = FindMarkers(seurat_obj, ident.1 = celltype1, ident.2 = celltype2, min.pct = 0.1, logfc.threshold = 0, test.use = "MAST", verbose = F)})
    }
...
if(!file.exists("./results/10X_5CL/sc_false_DEG_list.rds")){
  ...
  gene_filter = gene_selection(gene_bc_mat, 10)
  sc_gene = rownames(gene_bc_mat)[gene_filter]
  cell_1 = colnames(gene_bc_mat)[grep("A549", colnames(gene_bc_mat))]
  ...
  for(ii in seq(nrow(df))){
    ...
    cn2 = df[ii, "n2"]
    set.seed(12345)
    id = sample(seq(length(cell_1)), cn1 + cn2)
    this_name = paste0(cn1, '_', cn2)
    for(jj in method_name){
      if(jj == "Raw"){
        expr = gene_bc_mat[sc_gene, cell_1][, id]
      }else{
        gene_bc_mat = readh5_loom(paste0("./data/10X_5CL/", jj, ".loom"))
        expr = gene_bc_mat[sc_gene, cell_1][, id]
      }
