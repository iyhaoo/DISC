...
result_list = list()
this_output_dir = paste0(output_dir, "/split_path/", this_method)
...
for(ii in path_name){
  ...
  acc = acc_function(cell_order, order_list[["correct"]][[ii]], order_list[["wrong"]][[ii]])
  result_list[[ii]] = list(cds = cds, cell_order = cell_order, acc = acc)
...
saveRDS(result_list, paste0(output_dir, "/", this_method, "_monocle2_split_result.rds"))
...
for(ii in method_names){
  result_list = readRDS(paste0(output_dir, "/", ii, "_monocle2_split_result.rds"))
  for(jj in path_name){
    pair_list[[jj]][[ii]] = result_list[[jj]][["cell_order"]]
    cat(ii, " - ", jj, " - ", result_list[[jj]][["acc"]], "\n")
  }
}
for(ii in path_name){
  compare_pairs = get_compare_pairs(pair_list[[ii]])
  acc_all = c()
  for(jj in names(pair_list[[ii]])){
    acc_all = c(acc_all, acc_function(pair_list[[ii]][[jj]], order_list[["correct"]][[ii]], order_list[["wrong"]][[ii]], compare_pairs))
  }
  names(acc_all) = names(pair_list[[ii]])
  print(ii)
  print(acc_all)
  pair_list1 = list(DISC = pair_list[[ii]][["DISC"]], raw = pair_list[[ii]][["raw"]])
  compare_pairs1 = get_compare_pairs(pair_list1)
  acc_all1 = c()
  for(jj in names(pair_list1)){
    acc_all1 = c(acc_all1, acc_function(pair_list1[[jj]], order_list[["correct"]][[ii]], order_list[["wrong"]][[ii]], compare_pairs1))
  }
