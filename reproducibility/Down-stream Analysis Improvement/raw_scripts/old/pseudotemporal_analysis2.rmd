...
```
```{r}
cds = get_cds_monocle2(gene_bc_filt)
score = scorefunc_monocle2(cds)
result_list = list(cds = cds, score = score)
print(score[1] / (score[1] + score[2]))
saveRDS(result_list, paste0(output_dir, "/Raw_result_monocle2.rds"))
...
result_list = list(cds = cds, score = score)
print(score[1] / (score[1] + score[2]))
saveRDS(result_list, paste0(output_dir, "/DISC_result_monocle2.rds"))
```
###  monocle3

```{r}
library(monocle3)
get_earliest_principal_node = function(cds, root_cells){
  closest_vertex = cds@principal_graph_aux[["UMAP"]]$pr_graph_cell_proj_closest_vertex
  closest_vertex = as.matrix(closest_vertex[colnames(cds), ])
  return(igraph::V(principal_graph(cds)[["UMAP"]])$name[as.numeric(names(which.max(table(closest_vertex[root_cells, ]))))])
}
scorefunc_monocle3 <- function(cds) {
  if (length(cds@auxOrderingData[[cds@dim_reduce_type]]$branch_points) > 0) {
    sl <- NULL
    for(i in 1:length(cds@auxOrderingData[[cds@dim_reduce_type]]$branch_points)){
      tryCatch({cds_reduced <- buildBranchCellDataSet(cds,branch_point=i)},error=function(e) {})
...
get_cds_monocle3 = function(gene_bc_mat, root_cells){
  #  Firstly make a new CDS and use DDRTree for dimension reduction.
  pd = data.frame(row.names = colnames(gene_bc_mat), cell = colnames(gene_bc_mat))
  fd = data.frame(row.names = rownames(gene_bc_mat), gene_short_name = rownames(gene_bc_mat))
  cds = new_cell_data_set(gene_bc_mat, cell_metadata = pd, gene_metadata = fd)
  cds = preprocess_cds(cds, num_dim = 50, method = "PCA")
  cds = reduce_dimension(cds, reduction_method = "UMAP", preprocess_method = "PCA")
  cds = cluster_cells(cds, reduction_method = "UMAP")
  cds = learn_graph(cds)
  cds = order_cells(cds, reduction_method = "UMAP", root_pr_nodes = get_earliest_principal_node(cds, root_cells))
  aaa = pseudotime(cds, reduction_method = "UMAP")
  ...
  ggsave(paste0(output_dir, "/monocle3_DISC.pdf"), p)  
  
  
  p = plot_cell_trajectory(cds, color_by = "Pseudotime")
  ggsave(paste0(output_dir, "/monocle2_DISC.pdf"), p)  


  
  
  pData(cds)$CellType = cell_type[colnames(gene_bc_mat)]
  pData(cds)$level = as.character(ctlevel[cell_type[colnames(gene_bc_mat)], "level"])
  p = plot_cell_trajectory(cds, color_by = "CellType")
  ggsave(paste0(output_dir, "/monocle2_DISC_cell_type.pdf"), p)
  p = plot_cell_trajectory(cds, color_by = "level")
  ggsave(paste0(output_dir, "/monocle2_DISC_level.pdf"), p)  
...
```
```{r}
root_cells = names(which(cell_type == "HSC"))
cds = get_cds_monocle3(gene_bc_filt, root_cells)
score = scorefunc_monocle3(cds)
result_list = list(cds = cds, score = score)
print(score[1] / (score[1] + score[2]))
saveRDS(result_list, paste0(output_dir, "/Raw_result_monocle3.rds"))

###
gene_bc_mat = readh5_loom("./data/BONE_MARROW/DISC.loom")
gene_bc_filt = gene_bc_mat[used_genes, ]
cds = get_cds_monocle3(gene_bc_filt)
score = scorefunc_monocle3(cds)
result_list = list(cds = cds, score = score)
print(score[1] / (score[1] + score[2]))
saveRDS(result_list, paste0(output_dir, "/DISC_result_monocle3.rds"))
```
### TSCAN
```{r}
library(TSCAN)
...
```
```{r}
cds = get_cds_TSCAN(scran_normalization(gene_bc_filt))
score = scorefunc_TSCAN(cds$ord)
result_list = list(cds = cds, score = score)
print(score[1] / (score[1] + score[2]))
