...
get_score_monocle2 = function(cds, cell_type, correctorder, wrongorder){
  if(length(cds@auxOrderingData[[cds@dim_reduce_type]]$branch_points) > 0){
    ...
    for(i in 1:length(cds@auxOrderingData[[cds@dim_reduce_type]]$branch_points)){
      tryCatch({
        cds_reduced = buildBranchCellDataSet(cds,branch_point = i)
      },error = function(e){})
      ...
      sl = rbind(sl, rowSums(sapply(unique(df$Branch),function(x){
        ...
        c(sum(branch_cellorder %in% correctorder), sum(branch_cellorder %in% wrongorder))
      })))
    }
    used_sl = sl[which.max(sl[, 1] / rowSums(sl)), ]
    pair_number = sum(used_sl)
    acc = used_sl / pair_number
...
for(ii in method_names){
  ...
  pair_num_acc = get_score_monocle2(result_list[["cds"]], cell_type, unique(unlist(order_list[["correct"]])), unique(unlist(order_list[["wrong"]])))
