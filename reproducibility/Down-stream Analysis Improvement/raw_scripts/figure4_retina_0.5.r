...
for(ii in method_names){
  ...
  for(jj in as.character(c(0, 1, 3, 5, 6))){
    if(ii %in% c("Observed", "DISC", "SAVER", "MAGIC", "DCA", "scScope", "scVI")){
      ...
      cell_type_result = as.matrix(read.table(paste0("/home/wucheng/imputation/retina_analysis/ds1/repeat_", jj, "/0.5/", use_name, "/not_all_10/Recall1.txt"), sep = "\t"))
      overall_result = as.matrix(read.table(paste0("/home/wucheng/imputation/retina_analysis/ds1/repeat_", jj, "/0.5/", use_name, "/not_all_10/Accuracy.txt"), sep = "\t", header = T))
      ...
      all_result = readRDS(paste0("/home/yuanhao/data/fn/retina/ds/downsampling_first_repeat_", jj, "/imputation/cluster_evaluation_0.3/5_", use_name, "_mc_49_mce_1_resume_dim/pca/identification_result.rds"))
      this_result = list(Jaccard = all_result[["cell_type_result"]][, "Jaccard"], ACC = all_result[["summary"]]["ACC"])
      result_list[[ii]][[jj]] = this_result
    }
  }
}
output_dir = paste0("/home/yuanhao/data/fn/retina/ds/ds_0.5_results/clustering")
