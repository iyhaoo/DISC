...
for(ii in used_datasets){
  ...
  for(jj in repeats){
    ...
    DISC_cp10k = sweep(DISC_imputation, 2, 10000 / colSums(DISC_imputation), "*")
    ...
    data_list[[ii]][[jj]] = list(raw_zero_imputation = DISC_cp10k[raw_zero],
                                 induced_zero_imputation = DISC_cp10k[induced_zero],
                                 kept_values_imputation = DISC_cp10k[kept_values],
                                 top_raw_zero_imputation = DISC_cp10k[used_top_genes, ][raw_zero[used_top_genes, ]],
                                 top_induced_zero_imputation = DISC_cp10k[used_top_genes, ][induced_zero[used_top_genes, ]],
                                 top_kept_values_imputation = DISC_cp10k[used_top_genes, ][kept_values[used_top_genes, ]])
  }
}
```
...
for(ii in used_datasets){
  for(jj in repeats){
    ...
    y_max = quantile(c(density_raw_zero$y, density_induced_zero$y, density_kept_values$y), 0.99) * 1.02
    x_max = quantile(c(data_list[[jj]][[ii]][["top_raw_zero_imputation"]], data_list[[jj]][[ii]][["top_induced_zero_imputation"]], data_list[[jj]][[ii]][["top_kept_values_imputation"]]), 0.95) * 1.06
    plot(0, type = "n", xlim = c(-min(c(1, x_max * 0.1)), x_max), ylim = c(0, y_max), axes = FALSE, ann = FALSE, frame.plot = TRUE)
    lines(density_raw_zero, col = "black", lwd = 2)
    lines(density_induced_zero, col = "red", lwd = 2)
    lines(density_kept_values, col = "blue", lwd = 2)
